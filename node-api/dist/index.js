...
app.get('/products', token.verify, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const productos = yield mongodb.db.collection('carros').find({}).toArray();
...
app.get('/productCategoria/:categoria', token.verify, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const productos = yield mongodb.db.collection('carros').find({ "categoria": { $regex: ".*" + categoria + ".*" } }).toArray();
    res.status(200).json(apiUtils.BodyResponse(api_utils_1.APIStatusEnum.Success, 'OK', 'La solicitud ha tenido éxito', {
        productos,
        authUser: req.body.authUser
    }));
...
app.get('/search/:criterio/:parametro', token.verify, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    switch (busqueda) {
        case "descripcion":
            productos = yield mongodb.db.collection('carros').find({ "descripcion": { $regex: ".*" + parametro + ".*" } }).toArray();
            break;
        case "codigo":
            productos = yield mongodb.db.collection('carros').find({ "codigo": { $regex: ".*" + parametro + ".*" } }).toArray();
            break;
        case "precio":
            productos = yield mongodb.db.collection('carros').find({ "precio": { $regex: ".*" + parametro + ".*" } }).toArray();
            break;
        case "proveedor":
            productos = yield mongodb.db.collection('carros').find({ "proveedor": { $regex: ".*" + parametro + ".*" } }).toArray();
            break;
        case "provDescr":
            productos = yield mongodb.db.collection('carros').find({ "provDescr": { $regex: ".*" + parametro + ".*" } }).toArray();
            break;
    }
    res.status(200).json(apiUtils.BodyResponse(api_utils_1.APIStatusEnum.Success, 'OK', 'La solicitud ha tenido éxito', {
        productos,
...
app.get('/productId/:codigo', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const productos = yield mongodb.db.collection('carros').find({ "codigo": { $regex: ".*" + codigo + ".*" } }).toArray();
    res.status(200).json(apiUtils.BodyResponse(api_utils_1.APIStatusEnum.Success, 'OK', 'La solicitud ha tenido éxito', {
        productos,
        authUser: req.body.authUser
    }));
