...
app.post('/login', (req : Request, res : Response) => {
    ...
    const mockUser = {
        fullName: 'Ma Guadalupe Brizuela Gamiño',
        username: 'lupita',
        password: '1234',
        email: 'brizuelautl@gmail.com'
    }
...
app.get('/products', token.verify,async (req : Request, res : Response) => {
    ...
    const productos = await mongodb.db.collection('carros').find({ }).toArray();
...
app.get('/productCategoria/:categoria', token.verify,async (req : Request, res : Response) => {
    ...
    const productos = await mongodb.db.collection('carros').find({ "categoria" : { $regex : ".*" + categoria + ".*" }}).toArray();
...
app.get('/search/:criterio/:parametro', token.verify,async (req : Request, res : Response) => {
    ...
    {
        case "descripcion":
            productos = await mongodb.db.collection('carros').find({ "descripcion" : { $regex : ".*" + parametro + ".*" }}).toArray();
        break;
        case "codigo":
            productos = await mongodb.db.collection('carros').find({ "codigo" : { $regex : ".*" + parametro + ".*" }}).toArray();
        break;
        case "precio":
            productos = await mongodb.db.collection('carros').find({ "precio" : { $regex : ".*" + parametro + ".*" }}).toArray();
        break;
        case "proveedor":
            productos = await mongodb.db.collection('carros').find({ "proveedor" : { $regex : ".*" + parametro + ".*" }}).toArray();
        break;
        case "provDescr":
            productos = await mongodb.db.collection('carros').find({ "provDescr" : { $regex : ".*" + parametro + ".*" }}).toArray();
        break;
    }
...
app.get('/productId/:codigo', async (req: Request, res: Response) => {
    ...
    const productos = await mongodb.db.collection('carros').find({ "codigo" : { $regex : ".*" + codigo + ".*" }}).toArray();
